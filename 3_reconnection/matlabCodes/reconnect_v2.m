function [update_state, L_struct, new_six] = reconnect(label, new_six, L_struct, search_size, update_state, segmentation)
...
%                   
%                if ~(((0<LL) && (LL < 1000)) ||((0<LR) && (LR < 1000))  ||((0<RL) && (RL < 1000)) ||((0<RR) && (RR < 1000)))
%                    continue
%                end
%%  visualization                   
...
% 
%                 
%                 canvas_base = zeros(size(new_six(:,:,1)));
%                 canvas_base( L_struct(label).index) = 1;
%                 canvas_base = imdilate(canvas_base, ones(3));
%                 
%                 canvas5= (canvas2 + canvas + canvas4 + canvas_base*10);
%                  imshow(label2rgb(canvas5,'hsv', 'k', 'shuffle'),[]) ;
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                
%                 inter = canvas_base & canvas4;
              % ??? ind_dilate? 2044 * 2044(????, ?????? ????)
                    [row, col] = size(base.ind_dilate);
                if col ~= 1
                    base.ind_dilate = find(base.ind_dilate);
                end
                [row, col] = size(target.ind_dilate);
                if col ~= 1
                    target.ind_dilate = find(target.ind_dilate);
                end
                     [row, col] = size(base.index);
                if col ~= 1
                    base.index = find(base.index);
                end
                [row, col] = size(target.index);
                if col ~= 1
                    target.index = find(target.index);
                end               
                
                
                inter_num = numel(intersect(base.ind_dilate,target.ind_dilate));
                %% overlapping case;
                num_overlap = numel(intersect(base.index,target.index));
                if num_overlap ~= 0 || inter_num ~= 0
                    num_overlap_target_thin = numel(intersect(target.index, base.ind_dilate));
                    target_percent = num_overlap_target_thin / numel(target.index);
                    
                    num_overlap_base_thin = numel(intersect(base.index, target.ind_dilate));
                    base_percent = num_overlap_base_thin / numel(base.index);
                    [min_percent,the_min] = min([target_percent,base_percent]);
                    [large_percent,the_large] = max([target_percent,base_percent]);
                    
%                     imshow(label2rgb(canvas4 + canvas_base,'hsv', 'k', 'shuffle'),[]) ;
                    if large_percent > threshold
                        ...
                        elseif the_large == 2
                           ...
                           new_six(new_six==label) = 0;
                           new_six(new_six==related_label(i)) = label;
...
%                                  new_six(new_six ==related_label(i)) = 0;
                                 ...
                                 %%%%%% CHANGE PARA
                                 % print ('a left in b && b right in a')
...
%                                  new_six(new_six ==related_label(i)) = 0;
                         ...
                         %5 a's left in b && a's right in b
                         elseif(ismember(base_ind_lt, target.ind_dilate) && ismember(base_ind_rt, target.ind_dilate))
                             ...
                             new_six(new_six==related_label(i)) = label;
                             break;
                         elseif(ismember(tar_ind_lt, base.ind_dilate) && ismember(tar_ind_rt, base.ind_dilate))
                             L_struct(related_label(i)).exist = 0;
                             update_state = 1;
                             
                             new_six(new_six==related_label(i)) = 0;
                             break;            
                         %6 b's right in a && b's left in a 
                         end
%                          elseif((ismember(base_ind_lt, target.ind_dilate) && ismember(base_ind_rt, target.ind_dilate)) ...
        ...
        if ~isempty(record)
            ...
            if which_label ~= Inf
                ...
                %%%!!!!!!!!!!!!
                 ...
                 new_six(new_six==which_label) = label;
                 update_state = 1;
            end
        end
    end
